<x-admin-layout>
        ...
        <div class="max-w-4xl mx-auto">
            <div class="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-100">
                <div class="p-8 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.achievements.update', $achievement) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title', $achievement->title)" required autofocus />
                                <x-input-error class="mt-2" :messages="$errors->get('title')" />
                            </div>
                            <div>
                                <x-input-label for="title_en" value="Judul Prestasi (EN)" />
                                <x-text-input id="title_en" name="title_en" type="text" class="mt-1 block w-full" :value="old('title_en', $achievement->title_en)" />
                        ...
                        <div>
                            ...
                            <x-text-input id="slug" name="slug" type="text" class="mt-1 block w-full bg-gray-50 dark:bg-gray-900" :value="old('slug', $achievement->slug)" required />
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Slug akan menjadi link URL halaman ini.</p>
                        ...
                        <div>
                            ...
                            <x-text-input id="achievement_date" name="achievement_date" type="date" class="mt-1 block w-full" :value="old('achievement_date', $achievement->achievement_date ? $achievement->achievement_date->format('Y-m-d') : '')" />
                            <x-input-error class="mt-2" :messages="$errors->get('achievement_date')" />
                        </div>

                        <!-- Input Upload Gambar -->
                        <div>
                            <x-input-label for="image" :value="__('Foto/Sertifikat Prestasi (Biarkan kosong jika tidak ingin mengubah)')" />
                            @if($achievement->image_path)
                                <div class="mt-2 mb-3">
                                    <p class="text-sm text-gray-500 mb-2">Gambar Saat Ini:</p>
                                    <img src="{{ asset('storage/' . $achievement->image_path) }}" alt="Current Image" class="h-32 object-contain border border-gray-200 rounded p-1">
                                </div>
                            @endif
                        ...
                        <div class="flex items-center justify-end mt-8 gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700 focus:bg-primary-700 active:bg-primary-900 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                <i class="fas fa-save mr-2"></i> {{ __('Update Prestasi') }}
                            </button>
                        </div>
                    </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Hanya otomatis jika slug masih kosong/sama dengan title awal agar tidak mengubah url sembarangan jika cuma ganti judul minor
            let initialTitle = titleInput.value;
            
            titleInput.addEventListener('input', function () {
                if(slugInput.value === initialTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '')) {
