                ...
                <div class="p-8 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.posts.store') }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="slug" name="slug" type="text" class="mt-1 block w-full bg-gray-50 dark:bg-gray-900" :value="old('slug')" required placeholder="contoh-judul-berita" />
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Slug akan menjadi link URL berita Anda. Terisi otomatis dari judul.</p>
                            <x-input-error class="mt-2" :messages="$errors->get('slug')" />
                        </div>

                        <!-- Input Upload Gambar -->
                        <div>
                            <x-input-label for="image" :value="__('Gambar/Thumbnail Berita (Opsional)')" />
                        ...
                        <div class="flex items-center justify-end mt-8 gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700 focus:bg-primary-700 active:bg-primary-900 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                <i class="fas fa-save mr-2"></i> {{ __('Simpan Berita') }}
                            </button>
                        </div>
                    </form>
